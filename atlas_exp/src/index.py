...
class DistributedIndex(object):
    ...
    def load_index(self, path: str, total_saved_shards: int):
        ...
        ws = dist_utils.get_world_size()
        assert total_saved_shards % ws == 0, f"N workers must be a multiple of shards to save"
        shards_per_worker = 100 #total_saved_shards // ws
        passages = []
        embeddings = []
        batch_size = 10

        for batch_start in range(rank * shards_per_worker,60,batch_size):#(rank + 1) * shards_per_worker, batch_size):
            batch_end = min(batch_start + batch_size, (rank + 1) * shards_per_worker)
            embeddings_batch = []
            for shard_id in tqdm(range(batch_start,batch_end)):#, (rank + 1) * shards_per_worker)):
                passage_shard_path = self._get_saved_passages_path(path, shard_id)
                ...
                embeddings_batch.append(torch.load(embeddings_shard_path, map_location="cpu").cuda(1).half())
            embeddings.append(torch.concat(embeddings_batch,dim=1).to('cpu'))
        ...
        for chunk in passages:
            for p in chunk:
                ...
                n_passages += 1
        self.embeddings = torch.concat(embeddings, dim=1).to('cpu')
        del embeddings
    ...
    def _compute_scores_and_indices(self, allqueries: torch.tensor, topk: int) -> Tuple[torch.tensor, torch.tensor]:
        ...
        """
        scores = torch.matmul(allqueries.cuda(1).half(), self.embeddings.cuda(1))
